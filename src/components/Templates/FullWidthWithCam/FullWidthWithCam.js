...
function FullWidthWithCam() {
    ...
    const [cropVideoWidth, setCropVideoWidth] = useState(719)
    ...
    const handleSubmit = async (e) => {
        ...
        const cropVideo = `crop=${w2}:${h2}:${x2}:${y2},scale=720:405`

        const cropImg1 = 'crop=720:224.5:0:0'
        const cropImg2 = 'crop=720:224.5:0:1055.5'

        const overlay = `overlay=0:350`

        console.log(cropVideo, cropCam)
        ffmpeg.FS('writeFile', name1, await fetchFile(file1))

        ffmpeg.FS('writeFile', 'default_bg.png', await fetchFile(require('../../../assets/default_bg.png')))
        ...
        //crop img 1
        await ffmpeg.run('-i', 'default_bg.png', '-filter:v', cropImg1, '-c:v', 'libx264', '-preset', 'superfast', 'crop3.png')

        //crop img 2
        await ffmpeg.run('-i', 'default_bg.png', '-filter:v', cropImg2, '-c:v', 'libx264', '-preset', 'superfast', 'crop4.png')


        //stack the cropped clips
        await ffmpeg.run('-i', 'crop3.png', '-i', 'crop1.mp4', '-i', 'crop2.mp4', '-i', 'crop4.png', '-filter_complex', 'vstack=inputs=4', '-c:v', 'libx264', '-preset', 'superfast', 'output.mp4')
...
export default FullWidthWithCam